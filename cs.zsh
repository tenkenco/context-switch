...
_cs_save() {
  ...
  local name="" force=0 explicit_token="" use_clipboard=0 use_stdin=0
  while (($#)); do
    case "$1" in
    --force) force=1 ;;
    --token)
...
Usage:
  cs save <name> [--force]   Bootstrap a profile (paste setup-token + snapshot account).
...
How it works:
  ...
  - The `claude` wrapper additionally patches ~/.claude.json's oauthAccount
    ...
    right email. This is cosmetic; auth is the env var.
